sudo groupadd sftpadmins
sudo groupadd sftpusers
sudo usermod -a -G sftpadmins localadmin
sudo mkdir /media/data
sudo fdisk /dev/sdc
n
p
1
[default]
[default]
p
w
sudo mkfs -t ext4 /dev/sdc1
sudo -i blkid
[copy UUID of sdc1]
sudo nano /etc/fstab
UUID=339d6941-b4ff-46ac-a016-795c87f07667	/media/data	ext4	defaults,nofail	1	2
echo "alias update='sudo apt-get update && sudo apt-get upgrade -y && sudo apt-get dist-upgrade -y && sudo apt-get autoclean -y'" >> /home/localadmin/.bashrc
sudo reboot
update && sudo apt-get install samba
##sudo useradd -s /bin/true smbsvc
sudo smbpasswd -a localadmin
sudo nano /etc/samba/smb.conf
'''[data]
path = /mnt/data
available = yes
valid users = localadmin
read only = no
browsable = yes
public = yes
writable = yes'''
sudo service smbd restart
sudo useradd -g sftpusers --create-home --no-user-group scvlsftp
sudo useradd -g sftpusers --create-home --no-user-group vibessftp
sudo useradd -g sftpusers --create-home --no-user-group responsyssftp
sudo useradd -g sftpusers --create-home --no-user-group sfccsftp
sudo chmod 750 /media/data
sudo mkdir -p /media/data/scvl/inbound
sudo mkdir -p /media/data/scvl/outbound
sudo mkdir -p /media/data/vibes/inbound
sudo mkdir -p /media/data/vibes/outbound
sudo mkdir -p /media/data/responsys/inbound
sudo mkdir -p /media/data/responsys/outbound
sudo mkdir -p /media/data/sfcc/inbound
sudo mkdir -p /media/data/sfcc/outbound
sudo chown -R root:root /media/data/scvl
sudo chown -R root:root /media/data/vibes
sudo chown -R root:root /media/data/responsys
sudo chown -R root:root /media/data/sfcc
sudo chown scvlsftp:sftpadmins /media/data/scvl/inbound
sudo chown scvlsftp:sftpadmins /media/data/scvl/outbound
sudo chown scvlsftp:sftpadmins /media/data/vibes/inbound
sudo chown scvlsftp:sftpadmins /media/data/vibes/outbound
sudo chown scvlsftp:sftpadmins /media/data/responsys/inbound
sudo chown scvlsftp:sftpadmins /media/data/responsys/outbound
sudo chown scvlsftp:sftpadmins /media/data/sfcc/inbound
sudo chown scvlsftp:sftpadmins /media/data/sfcc/outbound
sudo chmod 770 /media/data/scvl/inbound
sudo chmod 770 /media/data/scvl/outbound
sudo chmod 770 /media/data/vibes/inbound
sudo chmod 770 /media/data/vibes/outbound
sudo chmod 770 /media/data/responsys/inbound
sudo chmod 770 /media/data/responsys/outbound
sudo chmod 770 /media/data/sfcc/inbound
sudo chmod 770 /media/data/sfcc/outbound
sudo nano /etc/ssh/sshd_config
'''Match user scvlsftp
	ForceCommand internal-sftp -u 7
	ChrootDirectory /media/data/scvl
	PermitTunnel no
	AllowAgentForwarding no
	AllowTcpForwarding no
	X11Forwarding no
Match user vibessftp
	ForceCommand internal-sftp -u 7
	ChrootDirectory /media/data/scvl
	PermitTunnel no
	AllowAgentForwarding no
	AllowTcpForwarding no
	X11Forwarding no
Match user responsyssftp
	ForceCommand internal-sftp -u 7
	ChrootDirectory /media/data/scvl
	PermitTunnel no
	AllowAgentForwarding no
	AllowTcpForwarding no
	X11Forwarding no
Match user sfccsftp
	ForceCommand internal-sftp -u 7
	ChrootDirectory /media/data/scvl
	PermitTunnel no
	AllowAgentForwarding no
	AllowTcpForwarding no
	X11Forwarding no'''
sudo mkdir /home/scvlsftp/.ssh
sudo mkdir /home/vibessftp/.ssh
sudo mkdir /home/responsyssftp/.ssh
sudo mkdir /home/sfccsftp/.ssh
sudo chown scvlsftp:sftpusers /home/scvlsftp/.ssh
sudo chown scvlsftp:sftpusers /home/vibessftp/.ssh
sudo chown scvlsftp:sftpusers /home/responsyssftp/.ssh
sudo chown scvlsftp:sftpusers /home/sfccsftp/.ssh
sudo sh -c "ssh-rsa  scvlsftp' > /home/scvlsftp/.ssh/authorized_keys"
sudo sh -c "ssh-rsa  scvlsftp-customer-provided' > /home/scvlsftp/.ssh/authorized_keys"
sudo sh -c "ssh-rsa  vibessftp' > /home/vibessftp/.ssh/authorized_keys"
sudo sh -c "ssh-rsa  responsyssftp' > /home/responsyssftp/.ssh/authorized_keys"
sudo sh -c "ssh-rsa  sfccsftp' > /home/sfccsftp/.ssh/authorized_keys"
sudo chown scvlsftp:sftpusers /home/scvlsftp/.ssh/authorized_keys
sudo chown scvlsftp:sftpusers /home/vibessftp/.ssh/authorized_keys
sudo chown scvlsftp:sftpusers /home/responsyssftp/.ssh/authorized_keys
sudo chown scvlsftp:sftpusers /home/sfccsftp/.ssh/authorized_keys
sudo systemctl restart sshd



sftp -o "IdentityFile=scvlsftp.ppk" scvlsftp@scvl-test-sftp-01.centralus.cloudapp.azure.com

"C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp"
"net use z: /delete"
"net use z: \\10.0.0.20\data Password20!7! /user:localadmin"
