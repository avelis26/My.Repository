...
SELECT EndDate,StoreNumber,Country_Cd
...
delete 
from [dbo].CEO_DB_FINAL
where enddate in (@curr_yr_date, @last_yr_date)


INSERT INTO CEO_DB_FINAL 
(EndDate, StoreNumber, Country_Cd, Member_Status, txn_cnt, totalamount)
select EndDate, agg2.StoreNumber, Country_Cd, agg2.Member_Status, sum(txn_cnt) txn_cnt, sum(totalamount) totalamount
from  [dbo].[CEO_StoreTxnItems] agg2  
group by agg2.EndDate, agg2.StoreNumber,Country_Cd, agg2.Member_Status

union all 

select EndDate, agg2.StoreNumber,Country_Cd, 'Total' as Member_Status, sum(txn_cnt) txn_cnt, sum(totalamount) totalamount
from   [dbo].[CEO_StoreTxnItems] agg2  
group by agg2.EndDate, Country_Cd , agg2.StoreNumber

--select * from CEO_DB_FINAL